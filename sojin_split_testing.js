...
var line_1 = new TextEvent ("Testing for TextEvent Object.");
...
function update (time, delta){
    if(!textDisplayDone)
        for (var individualLine in textOfLines){
            //console.log(individualLine);
            individualLine.y -= 0.015 * delta;
        }
}
...
function nextLine(game) {
    ...
    //create line object
    if(lineIndex === 0){
        window[currentLine] = game.add.text(350, 300, '', { font: "15px Courier New", fill: "#ffffff", lineSpacing: 15 });
    } else{
        window[currentLine] = game.add.text(350, 300-0.015*game.delta*game.time, '', { font: "15px Courier New", fill: "#ffffff", lineSpacing: 15 });
    }
        
    window[currentLine].text = line[wordIndex];
    console.log(window[currentLine]);

    textOfLines.push(window[currentLine]);
...
function nextWord(game) {
    ...
    if(line[wordIndex]) {
        window[currentLine].text = window[currentLine].text.concat(line[wordIndex]);
    }
    ...
    if (wordIndex === line.length)
    {
        ...
        window[currentLine].text = window[currentLine].text.concat("\n");
